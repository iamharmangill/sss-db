...
async function updatePost(todoItem, todoDateAdded, todoStatus, todoDueBy) {
  try {
    const updatetodoItem = await pool.query(`UPDATE todo SET todoDateAdded = '${todoDateAdded}', todoDateStatus = ${todoStatus}, todoDueBy = '${todoDueBy}' WHERE todoItem LIKE '%${todoItem}%';`)
